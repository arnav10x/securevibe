...
const INTRO: Record<string, string> = {
  all: 'Everything the scan found, worst first.',
...
export function FindingsBrowser({ findings }: { findings: BrowserFinding[] }) {
  ...
  // number on every tab.
  const numbers = new Map(ordered.map((f, i) => [f.id, i + 1]));
  const present = (['critical', 'high', 'medium', 'low'] as const).filter((s) =>
    ordered.some((f) => f.severity === s),
  );

  const [tab, setTab] = useState<string>(present[0] ?? 'all');
  const listRef = useRef<HTMLDivElement>(null);

  const visible = tab === 'all' ? ordered : ordered.filter((f) => f.severity === tab);
        ...
        {present.map((sev) => {
          ...
          return (
            <button
              ...
              onClick={() => setTab(sev)}
              aria-pressed={active}
              className={`flex cursor-pointer items-center gap-2.5 border-r border-[var(--line)] px-4 py-3 transition-all sm:px-5 ${
